...
library(data.table)
setwd("/Volumes/DATA/Seafile/NGS-Data-DC/PacBio/20210909 iSCREAM Run1/analysis")
...
referenceSeq <- "/Volumes/DATA/Seafile/NGS-Data-DC/PacBio/20210909 iSCREAM Run1/reference/pBG-ERBB3/ERBB3.fa"
...
data_analysis <- function(chr_mutation_vector){
  ...
  for(SAMPLE in searchSamples){
    ...
    FILE <-
      paste0(
        "/Volumes/DATA/Seafile/NGS-Data-DC/PacBio/20210909 iSCREAM Run1/analysis/sam2tsv/results/",
        SAMPLE,
        "_sam.tsv.gz"
      )
    Mut <- fread(file = FILE)
...
# reading file
iSCREAM <- readRDS("/Volumes/DATA/Seafile/NGS-Data-DC/PacBio/20210909 iSCREAM Run1/analysis/filtered/R.Binary/ERBB3_V1_iSCREAM.RDS")
...
idx <- sample(
  x = which(hits$FC >= 0.9 & hits$FC <= 1.1),
  size = 18,
  replace = F
)
hits <- hits[idx,]
